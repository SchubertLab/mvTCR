...
def objective(params, params_scgen, params_fixed):
	...
	params = helper.correct_params(params)
...
fixed_params['comet'] = True
fixed_params['name'] = f'bcc_tune_scgen_{fixed_params["model"]}{fixed_params["suffix"]}'
fixed_params['workspace'] = 'bcc-scgen'
fixed_params['metadata'] = ['patient', 'clonotype', 'cluster', 'cluster_tcr', 'treatment', 'response']
fixed_params['validate_every'] = 1
...
if fixed_params["grid_search"]:
	...
	algo = OptunaSearch(metric='scgen', mode='max', points_to_evaluate=init_params)
...
scgen_params = {
	...
	'indicator': 'pre',
	'hold_out': [('patient', 'su009'), ('patient', 'su006')],
	'per_column': None
}


analysis = tune.run(
	tune.with_parameters(objective, params_scgen=scgen_params, params_fixed=fixed_params),
