def suggest_params(trial):
    ...
    params = {
        ...
        'supervised': {
            'hidden_neurons': 2**trial.suggest_int('sv_hidden_neurons', 4, 8),
            'num_hidden_layers': trial.suggest_int('sv_hidden_neurons', 0, 4),
            'dropout': trial.suggest_float('sv_dropout', 0, 0.3, step=0.05),
            'activation': 'relu',
            'batch_norm': True,
            'loss_weights_sv': trial.suggest_float('loss_weights_sv', 1e-5, 1, log=True),
        }
    }
    return params
